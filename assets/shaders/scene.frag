...
struct Material {
    ...
    bool reflect_map_exists;
    sampler2D reflect_map;
};
...
void main() {
    if(material.constant) {
        ...
        if(skybox_exists) {
            ...
            if(material.reflect_map_exists) {
                reflection *= texture2D(material.reflect_map, texCoord);
            }

            if(material.diffuse_map_exists) {
                gl_FragColor = mix(vec4(result, texture2D(material.diffuse_map, texCoord).a), reflection, 0.5);
            } else {
                gl_FragColor = mix(vec4(result, 1.0), reflection, 0.5);
            }
        } else {
            if(material.diffuse_map_exists) {
