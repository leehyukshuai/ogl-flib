...
void main() {
    if(material.constant) {
        ...
        vec4 refraction = textureCube(skybox, R);

        if(material.diffuse_map_exists) {
            gl_FragColor = vec4(result, texture2D(material.diffuse_map, texCoord).a) + refraction;
        } else {
            gl_FragColor = vec4(result, 1.0) + refraction;
        }
    }
}
